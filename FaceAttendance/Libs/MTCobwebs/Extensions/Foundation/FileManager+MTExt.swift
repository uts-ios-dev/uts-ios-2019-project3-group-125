...
public extension FileManager {
    ...
    /// - Returns: 文件大小 单位:M
    static func fileSizeAtPath(_ filePath: String) -> Float {
    ...
    /// - Returns: 文件大小 单位:M
    static func folderSizeAtPath(_ folderPath: String) -> Float {
    ...
    static func diskOfAllSizeMBytes() -> CGFloat {
    ...
    static func diskOfFreeSizeMBytes() -> CGFloat {
        var size: CGFloat = 0.0
        do {
            let dic:Dictionary = try FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())
            let number = dic[.systemFreeSize] as? NSNumber
...
public extension FileManager {
    ...
    ///  返回指定路径单个文件的大小
    func getFileSizeWithPath(_ path: String) -> CGFloat {
    ...
    ///  返回指定路径文件夹的大小
    func getFolderSizeWithPath( path: String) -> CGFloat {
    ...
    /// 删除指定路径的cache 
    @discardableResult
    func clearFolderWithPath(_ path: String) -> Bool {
