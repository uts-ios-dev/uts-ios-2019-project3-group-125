...
public extension Date {
    ...
    //获取周
    static func getWeekDay(year: Int, month: Int, day: Int) -> Int {
...
public extension Date {
    ...
    /// - Returns:  结果字符串
    func timeAgoSince(_ date: Date = Date(), numericDates:Bool) -> String {
        let calendar = NSCalendar.current
    ...
    /// - Parameter date: 比较时间
    /// - Returns: 结果字符串
    func timeAfterTo(_ date:Date) -> String {
        let calendar = NSCalendar.current
        let now = Date()
...
public extension Date {
    ...
    /// weekday   1 当天
    /// - Returns: Date
    func endOfWeek(weekday: Int = 1) -> Date {
