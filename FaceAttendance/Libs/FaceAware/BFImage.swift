...
class BFImageView: UIImageView {
    ...
    func faceDetect(aImage: UIImage){
        let queue: DispatchQueue = DispatchQueue(label: "queuename", attributes: .concurrent)
        queue.async {
    ...
    func markAfterFaceDetect(features: [AnyObject], size: CGSize) {
        ...
        var fixedCenter: CGPoint = CGPoint(x: fixedRect.origin.x + fixedRect.size.width / 2.0, y:
